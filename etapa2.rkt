...
(define (ast-func suffixes)
  (cons (list (car (car suffixes))) (map (lambda (L)
                                           (cdr L))
                                         suffixes)))
...
(define (cst-func suffixes)
  (if (= (length suffixes) 1)
      suffixes
      (if (null? (longest-common-prefix-of-list suffixes))
          suffixes
          (cons (longest-common-prefix-of-list suffixes) (map (lambda (W)
                                                        (is-partially-included (longest-common-prefix-of-list suffixes) W))
                                                      suffixes)))))
...
(define (build-ast-tree suffixes alphabet)
  (foldr (lambda (ch acc)
           (if (null? (get-ch-words suffixes ch))
               acc
               (cons (cons (car (ast-func (get-ch-words suffixes ch))) (build-ast-tree (cdr (ast-func (get-ch-words suffixes ch))) alphabet)) acc)))
         '()
         alphabet))

(define (build-cst-tree suffixes alphabet)
  (foldl (lambda (ch acc)
           (if (null? (get-ch-words suffixes ch))
                   acc
                  (append acc (list (append (list (car (cst-func (get-ch-words suffixes ch)))) (build-cst-tree (cdr (cst-func (get-ch-words suffixes ch))) alphabet))))))
         '()
         alphabet))

(define (suffixes->st labeling-func suffixes alphabet)
  (if (equal? labeling-func ast-func)
     (build-ast-tree suffixes alphabet)
     (build-cst-tree suffixes alphabet)))
...
(define (text->st text)
  (lambda (acc)
    (let* ((suffixes (append acc (list #\$)))
           (sorted-alphabet (sort (remove-duplicates (cons #\$ acc)) char<?)))
      (suffixes->st text (get-suffixes suffixes) sorted-alphabet))))
